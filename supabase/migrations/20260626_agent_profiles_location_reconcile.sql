...
drop function if exists public.get_my_agent_status();
...
as $$
  ...
  order by
    ...
    case when not has_location_signal then freshness_at end desc nulls last,
    case when has_location_signal then freshness_at end desc nulls last,
    random_unit asc,
    listing_id
  offset greatest(coalesce(p_page, 0), 0) * greatest(least(coalesce(p_limit, 20), 50), 1)
  limit greatest(least(coalesce(p_limit, 20), 50), 1);
...
drop policy if exists "agent_profile_photos_public_read" on storage.objects;
create policy "agent_profile_photos_public_read"
on storage.objects
for select
