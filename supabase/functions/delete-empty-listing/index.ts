...
Deno.serve(async (request) => {
  ...
  if (agentError || !agent) {
    return json({ error: "Agent account not found" }, 403);
  }

  if (agent.account_status !== "active") {
    return json({ error: "Inactive or suspended agents cannot delete listings" }, 403);
  }
  ...
  if (listing.agent_id !== agent.id) {
    return json({ error: "You can delete only your own listings" }, 403);
  }
  ...
  if ((inquiryCount ?? 0) > 0) {
    return json(
      { error: "Listings with inquiries cannot be permanently deleted" },
      409,
    );
  }
  ...
  await serviceClient.from("audit_logs").insert({
    actor_id: user.id,
    action: "agent_deleted_empty_listing",
